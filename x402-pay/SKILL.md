...
- **Balance < service price** → fund it: Read `references/near-intents-funding.md` for the cross-chain funding flow. Always use NEAR intents to fund the wallet if the balance is low. Discover the funding sources available — from your own context (system prompt, env vars, config, known wallets) and an external-wallet option — present them to the user, and let them choose which to fund from.
