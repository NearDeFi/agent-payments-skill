// search-bazaar.mjs tests — verifies keyword search, listing, detail lookup, and source flag.
//
// Tests:
...
//   5. search --source bazaar: hits Coinbase bazaar, returns at least one price line

import { test } from 'node:test';
import assert from 'node:assert/strict';
import { run } from './helpers.mjs';

test('search-bazaar: finds results from x402-list', { timeout: 20_000 }, async () => {
  const { code, stdout } = await run('search-bazaar.mjs', ['search']);
  assert.equal(code, 0);
...
test('search-bazaar: --all includes offline services', { timeout: 20_000 }, async () => {
...
test('search-bazaar: details returns service metadata from x402-list', { timeout: 20_000 }, async () => {
  const { code, stdout } = await run('search-bazaar.mjs', ['details', 'https://x402.robtex.com']);
  assert.equal(code, 0);
...
test('search-bazaar: --source bazaar returns results from Coinbase bazaar', { timeout: 30_000 }, async () => {
  const { code, stdout } = await run('search-bazaar.mjs', ['search', '--all', '--source', 'bazaar']);
  assert.equal(code, 0);
